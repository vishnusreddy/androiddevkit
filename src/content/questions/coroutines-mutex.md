...
**Soundbite:** "Don't block threads with `synchronized` in coroutines — prefer immutable state + `StateFlow.update`, fall back to `Mutex.withLock` (suspends, non-reentrant) or single-thread confinement."
