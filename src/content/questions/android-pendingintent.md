...
**Soundbite:** "A PendingIntent lets another app act as you later. Default to `FLAG_IMMUTABLE`; only use `FLAG_MUTABLE` (with an explicit Intent) when a system feature like direct-reply must fill in data — otherwise it's an intent-redirection risk."
