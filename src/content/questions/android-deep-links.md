...
**Soundbite:** "Deep links are `ACTION_VIEW` intent filters; App Links add `autoVerify` + a hosted `assetlinks.json` so verified http(s) links open your app directly without a chooser. Use the Nav component's deep-link support to route and build the back stack."
