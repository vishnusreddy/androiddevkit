...
**Why interviewers ask:** the classic bug is building a flow with `onEach`/`map` and wondering why the side effects never fire — there's no terminal operator, so collection never starts. "Cold flows do nothing until collected" is the point being tested.
