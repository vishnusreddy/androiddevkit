...
**Why interviewers ask:** SOLID underpins *why* we use repositories, interfaces, DI, and layered architecture. The strongest answers tie each principle to a concrete Android decision (the examples above), not just recite definitions.

**Soundbite:** "SOLID: one responsibility per class, extend without modifying, subtypes honor contracts, small focused interfaces, and depend on abstractions. On Android they justify ViewModels, sealed types, fakes-for-testing, narrow callbacks, and interface-based DI."
