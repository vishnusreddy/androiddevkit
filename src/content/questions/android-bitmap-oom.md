...
**Soundbite:** "Bitmap memory is `w×h×bytesPerPixel`, not file size — downsample with `inSampleSize` to the target size, use `RGB_565` when alpha isn't needed, decode off-main-thread, and in practice let Coil/Glide handle sizing, caching, pooling, and cancellation."
