...
**Soundbite:** "Cold start builds the process + Application + first Activity — the one to optimize. Trim `Application.onCreate`, lazy-init SDKs, add Baseline Profiles and the Splash Screen API, load data async, and measure with Macrobenchmark/`am start -W`."
