---
question: "How does exception handling work in coroutines? Where do exceptions propagate for launch vs async?"
topic: coroutines
difficulty: senior
...
**Soundbite:** "`launch` throws into the hierarchy (handle with a `CoroutineExceptionHandler` or inner try/catch); `async` stores the exception and rethrows at `await()`."
