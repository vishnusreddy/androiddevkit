...
**Why interviewers ask:** it explains *why* the rules exist — why composables must be side-effect-free and idempotent (they re-run), why identity is positional (slot table), and why stability enables skipping.

**Soundbite:** "The compiler threads a `Composer` and group calls through each composable; the slot table stores state by call-site position (positional memoization), which is why `remember` works and why `key()` is needed to fix identity in loops."
